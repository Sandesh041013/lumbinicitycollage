...
const Navbar = () => {
  ...
  return (
    <header className="w-full bg-[#F5F6FA1A] backdrop-blur-md sticky top-0 z-50 shadow-sm">
      <div className="container w-11/12 mx-auto py-4 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center space-x-20 relative">
          {navLinks.map((link, index) => (
            ...
            >
              <div className="flex items-center text-[#1D2530] cursor-pointer hover:text-[#EBA747]">
                {link.href ? (
          ...
          <Link
            href="/enroll"
            className="ml-4 px-6 py-2.5 text-white  bg-[#2E3192] rounded-full hover:bg-[#3D220F] hover:text-white transition"
          >
            Apply Now
          </Link>
        </nav>
